...
CREATE OR ALTER PROCEDURE sp_ThongKeDeTaiTheoNam
AS
BEGIN
    SELECT 
        DETAI.NAM_TH,
        COUNT(*) AS SoLuongDeTai
    FROM DETAI
    GROUP BY DETAI.NAM_TH;
END
GO

CREATE OR ALTER PROCEDURE sp_ThongKeChiPhiDeTai
AS
BEGIN
    SELECT 
        DETAI.MADETAI,
...
CREATE OR ALTER PROCEDURE sp_ThongKeChiPhiTheoNam
AS
BEGIN
    SELECT 
        DETAI.NAM_TH,
...
CREATE OR ALTER PROCEDURE sp_ThongKeSoLuongGVTheoKhoa
AS
BEGIN
    SELECT 
        TENKHOA,
        SOLUONG_GV_KHOA AS SoGV_NCKH
    FROM KHOA;
END
GO

CREATE OR ALTER PROCEDURE sp_ThongKeSoLuongDeTaiTheoCap
AS
BEGIN
    SELECT 
        CAP_DETAI.TENCAP,
        COUNT(DETAI.MADETAI) AS SoLuongDeTai
    FROM DETAI, CAP_DETAI
    WHERE DETAI.MACAP = CAP_DETAI.MACAP
    GROUP BY CAP_DETAI.TENCAP;
END
GO

CREATE OR ALTER PROCEDURE sp_ThongKeKetQuaDanhGia
AS
BEGIN
    SELECT 
        DETAI.MADETAI,
        DETAI.TENDETAI,
        AVG(DANHGIA.DIEM) AS DiemTrungBinh
    FROM DETAI, DANHGIA
    WHERE DETAI.MADETAI = DANHGIA.MADETAI
    GROUP BY DETAI.MADETAI, DETAI.TENDETAI;
END
GO

CREATE OR ALTER PROCEDURE sp_ThongKeTienDoDeTai
AS
BEGIN
    SELECT 
        KETQUA.TIENDO,
        COUNT(KETQUA.MADETAI) AS SoLuongDeTai
    FROM KETQUA
    GROUP BY KETQUA.TIENDO;
END
GO

EXEC sp_ThongKeDeTaiTheoNam;
EXEC sp_ThongKeChiPhiDeTai;
EXEC sp_ThongKeChiPhiTheoNam;
EXEC sp_ThongKeSoLuongGVTheoKhoa;
EXEC sp_ThongKeSoLuongDeTaiTheoCap;
EXEC sp_ThongKeKetQuaDanhGia;
EXEC sp_ThongKeTienDoDeTai;

CREATE OR ALTER TRIGGER trg_KhongChoChiPhiSauHoanThanh
ON CHIPHI
INSTEAD OF INSERT
AS
BEGIN
    INSERT INTO CHIPHI(MACHI, MADETAI, NGAYCHI, SOTIENCHI, NOIDUNG_CHI, MOTA_CHI)
    SELECT i.MACHI, i.MADETAI, i.NGAYCHI, i.SOTIENCHI, i.NOIDUNG_CHI, i.MOTA_CHI
    FROM INSERTED i
    WHERE NOT EXISTS (
        SELECT 1 
        FROM KETQUA k
        WHERE k.MADETAI = i.MADETAI
          AND k.TIENDO = N'Hoàn thành'
    );

    -- Thông báo những bản ghi bị loại
    IF EXISTS (
        SELECT 1
        FROM INSERTED i
        WHERE EXISTS (
    ...
    BEGIN
        PRINT N'Một số chi phí không được thêm vì đề tài đã hoàn thành!';
    END
END
GO
select *from CHIPHI
select * from KETQUA
-- Thêm chi phí cho đề tài đang thực hiện (không in cảnh báo)
INSERT INTO CHIPHI(MACHI, MADETAI, NGAYCHI, SOTIENCHI, NOIDUNG_CHI, MOTA_CHI)
VALUES ('CP01', 'DT01', GETDATE(), 1000, N'Chi phí thiết bị', N'Mua linh kiện A');

-- Thêm chi phí cho đề tài đã hoàn thành (sẽ in cảnh báo)
INSERT INTO CHIPHI(MACHI, MADETAI, NGAYCHI, SOTIENCHI, NOIDUNG_CHI, MOTA_CHI)
VALUES ('CP02', 'DT02', GETDATE(), 2000, N'Chi phí nhân công', N'Thưởng nhóm hoàn thành')


CREATE OR ALTER TRIGGER trg_XoaDeTai
ON DETAI
FOR DELETE
AS
BEGIN
    DELETE FROM CHIPHI WHERE MADETAI IN (SELECT MADETAI FROM DELETED);
    DELETE FROM KETQUA WHERE MADETAI IN (SELECT MADETAI FROM DELETED);
    DELETE FROM NHOM_DETAI WHERE MADETAI IN (SELECT MADETAI FROM DELETED);
    DELETE FROM DANHGIA WHERE MADETAI IN (SELECT MADETAI FROM DELETED);
    DELETE FROM KETQUA_DANHGIA WHERE MADETAI IN (SELECT MADETAI FROM DELETED);
END
GO

CREATE OR ALTER TRIGGER trg_TangSoLuongGVNCKH
ON THANHVIEN_NHOM
FOR INSERT
AS
BEGIN
    UPDATE KHOA
    SET SOLUONG_GV_KHOA = SOLUONG_GV_KHOA + 1
    FROM KHOA, GIANGVIEN, INSERTED
    WHERE KHOA.MAKHOA = GIANGVIEN.MAKHOA
      AND GIANGVIEN.MAGV = INSERTED.MAGV;
    PRINT N'Số lượng giảng viên trong khoa đã được cập nhật sau khi thêm thành viên nhóm.';
END
GO

CREATE OR ALTER TRIGGER trg_GiamSoLuongGVNCKH
ON THANHVIEN_NHOM
FOR DELETE
AS
BEGIN
    UPDATE KHOA
    SET SOLUONG_GV_KHOA = SOLUONG_GV_KHOA - 1
    FROM KHOA, GIANGVIEN, DELETED
    WHERE KHOA.MAKHOA = GIANGVIEN.MAKHOA
      AND GIANGVIEN.MAGV = deleted.MAGV;

    PRINT N'Số lượng giảng viên trong khoa đã được cập nhật sau khi xoá thành viên nhóm.';
END
GO

CREATE OR ALTER TRIGGER trg_CapNhatDiemTrungBinh
ON DANHGIA
...
BEGIN
    ...
    SET DIEM_TB = (
        SELECT AVG(DIEM)
        FROM DANHGIA d
        WHERE d.MADETAI = k.MADETAI
    )
    FROM KETQUA_DANHGIA k
    WHERE k.MADETAI IN (
        SELECT MADETAI FROM INSERTED
        UNION
        SELECT MADETAI FROM DELETED
    );
END
GO

CREATE OR ALTER TRIGGER trg_KhongDanhGiaTrung
ON DANHGIA
FOR INSERT
AS
BEGIN
    -- Chèn các đánh giá chưa tồn tại
    INSERT INTO DANHGIA(MATHANHVIEN, MADETAI, DIEM, NHANXET)
    SELECT i.MATHANHVIEN, i.MADETAI, i.DIEM, i.NHANXET
    FROM INSERTED i
    WHERE NOT EXISTS (
        SELECT 1
        FROM DANHGIA d
        WHERE d.MATHANHVIEN = i.MATHANHVIEN
          AND d.MADETAI = i.MADETAI
    );

    -- Thông báo nếu có bản ghi bị loại
    IF EXISTS (
        SELECT 1
        FROM INSERTED i
        WHERE EXISTS (
    ...
    BEGIN
        PRINT N'Một số đánh giá bị loại vì thành viên đã đánh giá đề tài này!';
    END
END
GO
PRINT '--- TEST: chèn đánh giá hợp lệ ---';
INSERT INTO DANHGIA(MATHANHVIEN, MADETAI, DIEM, NHANXET)
VALUES
('TV01', 'DT01', 8, N'Tốt'),
('TV02', 'DT01', 9, N'Khá');
...
PRINT '--- TEST: chèn đánh giá trùng ---';
-- TV01 đánh giá DT01 lần nữa → trigger sẽ loại và in thông báo
INSERT INTO DANHGIA(MATHANHVIEN, MADETAI, DIEM, NHANXET)
VALUES
('TV01', 'DT01', 10, N'Xuất sắc');
