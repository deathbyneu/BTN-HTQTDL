...
CREATE OR ALTER PROCEDURE sp_ThongKeTongChiPhi
AS
BEGIN
    SELECT 
        DETAI.MADETAI,
...
CREATE OR ALTER PROCEDURE sp_ThongKeSL_GV_Nhom
AS
BEGIN
    SELECT 
        NHOM_DETAI.MANHOM,
        NHOM_DETAI.TENNHOM,
        COUNT(THANHVIEN_NHOM.MAGV) AS SoLuongGV
    FROM NHOM_DETAI, THANHVIEN_NHOM
    WHERE NHOM_DETAI.MANHOM = THANHVIEN_NHOM.MANHOM
    GROUP BY NHOM_DETAI.MANHOM, NHOM_DETAI.TENNHOM
END
GO

CREATE OR ALTER PROCEDURE sp_ThongKeDeTai_TheoLinhVuc
AS
BEGIN
    SELECT 
        LINHVUC.TENLINHVUC,
    ...
    FROM DETAI, LINHVUC
    WHERE DETAI.MALINHVUC = LINHVUC.MALINHVUC
    GROUP BY LINHVUC.TENLINHVUC
END
GO

CREATE OR ALTER PROCEDURE sp_DeTaiHoanThanh
AS
BEGIN
    SELECT 
        DETAI.MADETAI,
        ...
        KETQUA.TIENDO
    FROM DETAI, KETQUA
    WHERE DETAI.MADETAI = KETQUA.MADETAI
      AND KETQUA.TIENDO = N'HOÀN THÀNH'
END
GO
exec sp_DeTaiHoanThanh
CREATE OR ALTER PROCEDURE sp_ThongKeDiemTB
AS
BEGIN
    SELECT 
        DETAI.MADETAI,
        DETAI.TENDETAI,
        AVG(DANHGIA.DIEM) AS DIEM_TB
    FROM DETAI, DANHGIA
...
GO
ALter table DETAI 
ADD TONG_CHIPHI INT NULL
SELECT * from DETAI
CREATE OR ALTER TRIGGER trg_UpdateTotalCost
ON CHIPHI
AFTER INSERT
AS
BEGIN
    -- Cập nhật tổng chi phí cho tất cả đề tài có trong INSERTED
    UPDATE DETAI
    SET TONG_CHIPHI = (
        SELECT SUM(SOTIENCHI)
        FROM CHIPHI
        WHERE CHIPHI.MADETAI = DETAI.MADETAI
    )
    WHERE MADETAI IN (SELECT DISTINCT MADETAI FROM INSERTED);

    -- Thêm mới các đề tài CHƯA có trong bảng TONGCHIPHI
    INSERT INTO DETAI(MADETAI, TONG_CHIPHI)
    SELECT I.MADETAI,
           (SELECT SUM(SOTIENCHI) FROM CHIPHI WHERE MADETAI = I.MADETAI)
    FROM (SELECT DISTINCT MADETAI FROM INSERTED) AS I
    WHERE NOT EXISTS (
        SELECT 1 FROM DETAI T WHERE T.MADETAI = I.MADETAI
    );
END
GO
INSERT INTO CHIPHI(MACHI, MADETAI, NGAYCHI, SOTIENCHI, NOIDUNG_CHI)
VALUES ('TEST01', 'DT01', '2025-01-01', 50000, N'TEST CHI');ss
SELECT * FROM DETAI

CREATE OR ALTER TRIGGER trg_UpdateSLThanhVien_Add
ON THANHVIEN_NHOM
AFTER INSERT
AS
BEGIN
    ...
    SET SOLUONG_GV_NHOM = (
        SELECT COUNT(*)
        FROM THANHVIEN_NHOM
        WHERE MANHOM = NHOM_DETAI.MANHOM
    )
    WHERE MANHOM IN (SELECT DISTINCT MANHOM FROM INSERTED)
END
GO


CREATE OR ALTER TRIGGER trg_UpdateTotalCost_Delete
ON CHIPHI
AFTER DELETE
AS
BEGIN
    ...
    WHERE MADETAI IN (SELECT DISTINCT MADETAI FROM DELETED)
END
GO



EXEC sp_ThongKeTongChiPhi
EXEC sp_ThongKeSL_GV_Nhom
EXEC sp_ThongKeDeTai_TheoLinhVuc
EXEC sp_ThongKeDiemTB
EXEC sp_DeTaiHoanThanh
