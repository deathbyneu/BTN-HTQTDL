...
BEGIN
    ...
    SELECT @Ma = MADETAI FROM INSERTED

    IF EXISTS (SELECT 1 FROM KETQUA_DANHGIA WHERE MADETAI = @Ma)
        UPDATE KETQUA_DANHGIA
        SET DIEM_TB = DIEM_TB
        WHERE MADETAI = @Ma
    ELSE 
...
BEGIN
    DECLARE @Ma CHAR(10)
    SELECT @Ma = MANHOM FROM INSERTED
...
CREATE OR ALTER TRIGGER trg_UpdateSLThanhVien_Delete
ON THANHVIEN_NHOM
AFTER DELETE
AS
BEGIN
    DECLARE @Ma CHAR(10)
    SELECT @Ma = MANHOM FROM DELETED
...
CREATE OR ALTER TRIGGER trg_UpdateDiemTB
ON DANHGIA
